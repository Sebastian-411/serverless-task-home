...
class Task extends BaseEntity {
  constructor({
    ...
    this._setCompletedAt(completedAt);
  ...
  /**
   * Sets and validates task owner
   * @private
   * @param {string} userId - User ID who owns the task
   */
  _setUserId(userId) {
    this._validateUUID(userId, 'userId');
    this.userId = userId;
  }
   ...
   * @private
   * @param {string|Date} completedAt - Completion timestamp
   */
  _setCompletedAt(completedAt) {
    if (completedAt) {
      this._validateTimestamp(completedAt, 'completedAt');
   ...
   */
  isHighPriority() {
    return this.priority === TASK_PRIORITY.HIGH || this.priority === TASK_PRIORITY.URGENT;
  }
  ...
  complete() {
    ...
    if (this.status === TASK_STATUS.CANCELLED) {
  ...
  reopen() {
    if (this.status !== TASK_STATUS.COMPLETED && this.status !== TASK_STATUS.CANCELLED) {
      throw new ValidationError('Only completed or cancelled tasks can be reopened');
    }
