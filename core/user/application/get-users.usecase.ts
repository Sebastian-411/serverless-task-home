...
export class GetUsersUseCase {
  ...
  async execute(
    ...
    console.log("[GetUsersUseCase][execute] Get users request received", {
      authUserId: authContext?.user?.id,
      authUserRole: authContext?.user?.role,
      pagination,
    });
    try {
      ...
      if (!authContext.isAuthenticated || !authContext.user) {
        console.warn(
          "[GetUsersUseCase][execute] Validation failed: Authentication required",
        );
      ...
      if (
        authContext.user.role.toLowerCase() !== UserRole.ADMIN.toLowerCase()
      ) {
        console.warn(
          "[GetUsersUseCase][execute] Validation failed: Only administrators can access the users list",
          { authUserId: authContext.user.id },
        );
      ...
      if (!pagination || (page === 1 && limit === 10)) {
        ...
        if (cachedUsers) {
          console.log(
            "[GetUsersUseCase][execute] Cache HIT - Users list served",
            { adminEmail: authContext.user.email },
          );
          return {
            users: cachedUsers,
      ...
      console.log(
        "[GetUsersUseCase][execute] Cache MISS - Fetching users from database",
        { adminEmail: authContext.user.email, page, limit },
      );
      ...
      console.log("[GetUsersUseCase][execute] Users fetched and cached", {
